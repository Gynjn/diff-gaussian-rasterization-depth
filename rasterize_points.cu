...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& conf,       //parameters
	...
	const bool debug)
{
  ...
  torch::Tensor out_confmap = torch::full({NUM_CHANNELS_CONF, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_confmap.contiguous().data<float>(),
        depth_map.contiguous().data<float>(),
        weight_map.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_confmap, radii, geomBuffer, binningBuffer, imgBuffer, depth_map, weight_map);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_confmap,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dconf = torch::zeros({P, NUM_CHANNELS_CONF}, means3D.options());
  ...
  {  
	  ...
	  dL_dout_confmap.contiguous().data<float>(),
      dL_dout_depth.contiguous().data<float>(),
	  dL_dmeans2D.contiguous().data<float>(),
	  dL_dconic.contiguous().data<float>(),  
	  dL_dopacity.contiguous().data<float>(),
	  dL_dcolors.contiguous().data<float>(),
	  dL_dconf.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dconf, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations);
}
